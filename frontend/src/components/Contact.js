...
try {
  ...
  if (submissionStatus === "pending") {
    message = <div className="text-center mb-3">Submitting...</div>;
  } else if (submissionStatus === "success") {
    message = <div className="text-center mb-3">Your data is submitted successfully.</div>;
  } else if (submissionStatus === "error") {
    message = <div className="text-center text-danger mb-3">Error submitting data!</div>;
  }
  return (
...
<form id="contactForm" action="POST" onSubmit={submit}>
  ...
  <div className="mb-3">
    ...
    <input className="form-control" onChange={(e)=>{setEmail(e.target.value)}}  id="emailAddress" type="email" placeholder="Email Address" data-sb-validations="required, email" />
